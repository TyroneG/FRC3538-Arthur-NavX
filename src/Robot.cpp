...
class Robot: public IterativeRobot  // FRC3538 Arthur NavX Code
{
...
private:
	...
	bool DriveFwd(std::string L, std::string M, float a, float spd = 1.0, bool Hgear = false)
	{
		...
		double Gvalue=ahrs->GetAngle();
		turningValue1 = (angleSetpoint - Gvalue) * -pGain;
	...
	bool TurnRight(std::string L, std::string M, double Tvalue=0, bool Hgear = false)
	{
		...
		Gvalue = ahrs->GetAngle();
		turningValue2 = (Tvalue - Gvalue) * -pGain;
